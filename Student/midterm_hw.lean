...
meant to turn by 120 degree and then to turn by another 240
...
To create an instance of the AddMonoid class for Rotations
...
/-!
We need to provide explicitly only proofs of the identity axioms!
...
instance : Sub Rotation := { sub := sub_Rotation }
...
-/
-- vsub is state subtraction

-- note from 2/28: need a subtraction operation--hence the subnegmonoid?
-- AddTorsor type class?
-- can use sorry instead of building proofs
-- need to implement AddAction?
-- G is rotation, P is State

-- Hint: follow the same approach

#check Nonempty

def vsub_rot_state : State → State → Rotation
| s0, s0 => r0
| s0, s120 => r240
| s0, s240 => r120
| s120, s0 => r120
...
instance : VSub Rotation State := ⟨ vsub_rot_state ⟩
